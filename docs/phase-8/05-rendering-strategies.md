...
이 문서는 web.dev의 Rendering on the Web 문서와 React 19.2의 hydration·streaming server rendering 문서를 기준으로 한다. 각 전략의 동작 모델과 요청 타임라인, 캐시 상태 전이는 [Phase 5b — React Rendering Patterns](../phase-5b/01-client-side-rendering.md)가 문서별로 소유한다. 이 문서는 그 선택을 [Core Web Vitals](./03-web-performance.md)와 브라우저·네트워크 계측으로 검증하는 비용 모델에 집중하고, Next.js 같은 프레임워크의 구체 API는 [8-6 Next.js와 RSC](./06-nextjs-and-rsc.md)에서 다룬다.
...
CSR(Client-Side Rendering)은 서버가 대개 HTML shell과 JavaScript 번들을 제공하고, 브라우저가 데이터를 가져와 DOM을 만든다. cold entry와 warm navigation의 타임라인 분리는 [5b-1 Client-side Rendering](../phase-5b/01-client-side-rendering.md)에서 세운 모델을 전제로 한다.
...
SSR(Server-Side Rendering)은 요청마다 서버가 React tree를 HTML로 렌더링해 보낸다. 브라우저는 HTML을 즉시 파싱하고 화면에 그릴 수 있다. snapshot 재현 계약과 hydration mismatch의 원인 분류는 [5b-2 Server-side Rendering](../phase-5b/02-server-side-rendering.md)에서 다뤘다.
...
SSG(Static Site Generation)는 가능한 URL의 HTML을 빌드 때 미리 만든다. 배포 산출물은 정적 파일이다. build snapshot과 artifact 일관성, route cardinality의 경계는 [5b-3 Static Rendering](../phase-5b/03-static-rendering.md)에서 다뤘다.
...
ISR(Incremental Static Regeneration)은 정적 HTML을 캐시하되, 일정 시간이나 특정 이벤트 뒤에 다시 생성한다. 프레임워크마다 구현은 다르지만 모델은 같다. cache entry 상태 머신과 invalidation·regeneration·propagation의 구분은 [5b-4 Incremental Static Generation](../phase-5b/04-incremental-static-generation.md)에서 다뤘다.
...
하이드레이션(hydration)은 서버가 보낸 HTML을 버리지 않고, 클라이언트 React가 같은 tree를 렌더해 이벤트 핸들러와 상태를 연결하는 과정이다. 활성화를 영역과 trigger로 나누는 전략은 [5b-5 Progressive Hydration](../phase-5b/05-progressive-hydration.md), React가 Suspense 단위로 순서를 조정하는 내장 동작은 [5b-8 Selective Hydration](../phase-5b/08-selective-hydration.md)에서 다뤘다.
...
스트리밍 SSR은 준비된 부분부터 HTML chunk를 보낸다. shell·boundary·오류·proxy buffering의 전달 계약은 [5b-6 Streaming SSR](../phase-5b/06-streaming-server-side-rendering.md)에서 다뤘다.
