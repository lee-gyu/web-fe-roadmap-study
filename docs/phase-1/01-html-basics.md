...
`value`도 같다. `getAttribute('value')`는 초기값을, `el.value`는 사용자가 타이핑한 현재값을 돌려준다. 지금은 "속성 = 초기 설정, 프로퍼티 = 살아있는 상태"라는 구분만 세워 두면 된다. DOM 조작은 Phase 2-5에서 본격적으로 다루고, React의 제어 컴포넌트(Phase 4-3)에서 이 구분이 다시 핵심이 된다.
