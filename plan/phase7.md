...
- **다루지 않을 범위**: 코드 리뷰 문장 작성법과 아키텍처 리뷰 상세(Phase 10), CI 설정 문법(6-5), release artifact 배포(7-8에서 Git 관점만)
...
- 뒤 Phase로 위임하는 주제: 코드 리뷰의 문장·설계 관점·리팩터링 전략은 10-2에서 본격적으로 다룬다. Phase 7은 Git 이력과 PR 정책이 리뷰를 가능하게 만드는 구조까지만 다룬다.
