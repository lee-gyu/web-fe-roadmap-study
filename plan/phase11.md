...
- **다룰 범위**:
  ...
  - 대표 에이전트의 간단한 지형: Codex는 OpenAI의 소프트웨어 개발용 코딩 에이전트로 CLI/IDE/앱/웹 표면을 갖춘 예시, Claude Code는 코드베이스 읽기·파일 편집·명령 실행을 중심으로 한 터미널/IDE/데스크톱/웹 에이전트 예시, Antigravity는 에이전트 우선 IDE와 다중 에이전트 조율 표면의 예시로 다룬다. Pi와 Hermes는 작성 시점에 공식 자료를 재확인한 뒤, 제품인지 하네스인지 연구 시스템인지 성격을 명확히 밝히고 짧은 비교 사례로만 사용한다.
...
- **다룰 범위**:
  ...
  - 병렬 작업: Git worktree 또는 격리된 브랜치로 충돌을 줄이고, 독립적인 버그 수정·문서화·테스트 보강에만 병렬 에이전트를 사용한다.
...
- Codex, Claude Code, Antigravity, Pi, Hermes의 성격을 간단히 비교한다. Pi와 Hermes는 작성 시점 공식 자료 또는 신뢰 가능한 1차/연구 자료로 성격을 확인하고, 모호하면 "확인 필요"로 남긴다.
...
- [ ] 작업 명세 템플릿 5종 이상 작성
- [ ] Codex, Claude Code, Antigravity, Pi, Hermes의 성격과 사용 범위 비교표 작성
...
- **제품 정보는 작성 시점 기준으로 확인**: Codex, Claude Code, Antigravity처럼 빠르게 바뀌는 도구는 공식 문서와 릴리스 노트를 확인하고 기준 날짜를 명시한다. Pi와 Hermes는 명칭이 모호할 수 있으므로 제품·하네스·연구 시스템 중 무엇인지 먼저 확인한 뒤 서술한다.
...
- [Polar: Agentic RL on Any Harness at Scale](https://arxiv.org/abs/2605.24220): 기존 하네스를 블랙박스로 실행하고 model API boundary에서 trace를 수집하는 관점을 확인한다. Pi는 이 논문에서 언급되는 harness 사례를 출발점으로 삼되, 별도 공식 자료를 재확인한다.
- [Making OpenAPI Documentation Agent-Ready](https://arxiv.org/abs/2605.14312): Hermes가 OpenAPI 문서와 REST API를 에이전트가 사용할 수 있게 점검하는 다중 에이전트 시스템으로 소개되는 맥락을 확인한다.
