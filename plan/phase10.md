# Phase 10 — 실전 프로젝트와 기술 검증 학습 과정 기획

> ROADMAP.md의 Phase 10(3주+, 문서 4개)를 실제 집필 가능한 수준으로 구체화한 기획 문서다.
...
독자는 5년차 이상 경력 개발자(백엔드·모바일 출신)로, Phase 0~9에서 웹 플랫폼, HTML/CSS 렌더링, HTTP, JavaScript 런타임, TypeScript 타입 설계, React 렌더링 모델, 프론트엔드 도구, Git 협업, 브라우저 성능·보안·Next.js/RSC, 설계 패턴까지 다뤘다. Phase 10은 새 API를 하나 더 배우는 과정이 아니라, 지금까지 익힌 원리와 판단 기준을 **완성된 프로젝트와 검증 가능한 기술 서사**로 전환하는 과정이다.
...
문서 4개는 세 블록으로 묶인다: **프로젝트 정의와 의사결정**(10-1), **품질 검증과 리뷰**(10-2), **기술 서사의 외부화**(10-3~10-4). 팀 프로젝트를 선택하면 3주 이후에도 구현과 리뷰 사이클을 반복할 수 있도록 확장 가능하게 설계한다.

| 주차 | 문서 | 실습 |
|------|------|------|
| 1주차 | 10-1 프로젝트 가이드 | 문제 정의, 요구사항·비기능 요구사항 작성, 기술 선택 후보 비교, ADR 초안, 마일스톤과 협업 규칙 수립 |
| 2주차 | 10-2 코드 품질과 리뷰 | 핵심 수직 절편(vertical slice) 구현, PR 단위 설계, 코드 리뷰 체크리스트 적용, 테스트·타입·성능·접근성·보안 검증 |
| 3주차 | 10-3 포트폴리오와 이력서, 10-4 면접 준비 | 배포, README와 사례 연구 작성, 이력서 bullet 정리, Phase별 원리 기반 면접 답변과 프론트엔드 시스템 설계 연습 |
...
각 문서는 CLAUDE.md의 공통 구조를 따른다. Phase 10 문서는 "좋은 조언"을 나열하는 대신, 앞선 Phase에서 만든 기술 모델을 실제 프로젝트 판단과 산출물로 연결해야 한다. 모든 선택은 대안·근거·측정 방법·무너지는 조건을 함께 다룬다.

### 10-1. 프로젝트 가이드 — `docs/phase-10/01-project-guide.md`
...
- **다루지 않을 범위**: 코드 리뷰의 세부 관점(10-2), README와 이력서 표현법(10-3), 면접 답변 구조(10-4), 프로젝트 관리 방법론 일반론.
...
- **의존**: Phase 5의 React 상태·렌더링 모델, Phase 6의 도구·CI, Phase 7의 Git 협업, Phase 8의 성능·보안·렌더링 전략, Phase 9의 설계 패턴. 특히 8-5/8-6의 CSR/SSR/SSG/RSC 판단과 9-5/9-6의 컴포넌트 패턴 판단을 프로젝트 ADR로 이어받는다.

### 10-2. 코드 품질과 리뷰 — `docs/phase-10/02-code-quality-and-review.md`
...
### 10-3. 포트폴리오와 이력서 — `docs/phase-10/03-portfolio-and-resume.md`
...
- **다루지 않을 범위**: 디자인 템플릿 제작, 일반 취업 조언, 지원 회사별 자기소개서, 면접 질의응답 상세(10-4).
- **경력자 연결**: 경력직 포트폴리오는 "사용한 기술 목록"보다 "제약 아래 의사결정을 내리고 결과를 검증한 기록"에 가깝다. 장애 회고나 기술 설계 문서처럼, 맥락과 판단 근거가 빠지면 산출물의 기술 깊이가 보이지 않는다.
- **의존**: 10-1의 ADR과 요구사항, 10-2의 리뷰·품질 기록, 8-3의 성능 리포트, 8-4의 보안 점검, 7-3의 커밋 설계와 7-7의 PR 흐름.

### 10-4. 면접 준비 — `docs/phase-10/04-interview-prep.md`
...
- **다룰 범위**:
  ...
  - Phase별 핵심 질문 맵: 브라우저 파이프라인, HTML 파서와 접근성, CSS 캐스케이드와 레이아웃, HTTP 캐싱·쿠키·TLS, JavaScript 이벤트 루프와 메모리, TypeScript 타입 설계, React 렌더링·상태·이펙트, 번들러·테스트·CI, Git 협업, 성능·보안·렌더링 전략, JavaScript/React 설계 패턴을 면접 질문 형태로 재구성한다.
...
- **의존**: Phase 0~9 전체와 10-1~10-3의 프로젝트 산출물. 특히 면접 답변은 추상 지식만으로 만들지 않고 본인이 작성한 ADR, 성능 리포트, 코드 리뷰 기록, README와 연결한다.
...
```mermaid
flowchart TD
    D1["10-1 프로젝트 가이드"] --> D2["10-2 코드 품질과 리뷰"]
    D1 --> D3["10-3 포트폴리오와 이력서"]
    D2 --> D3
    D1 --> D4["10-4 면접 준비"]
    D2 --> D4
    D3 --> D4
    ...
    P9["Phase 9 설계 패턴"] -.전제.-> D1
    P9 -.전제.-> D2
    P8 -.증거.-> D3
    P0["Phase 0~4 웹 플랫폼·HTTP·JS·TS"] -.면접 지식.-> D4
```

- 집필 순서는 번호 순서(10-1 → 10-4)를 그대로 따른다. 10-1이 프로젝트의 문제와 의사결정 구조를 세우고, 10-2가 구현 과정에서 품질을 검증한다. 10-3은 그 산출물을 외부 독자가 읽을 수 있는 포트폴리오와 이력서로 압축하고, 10-4는 같은 근거를 면접 답변과 시스템 설계 답변으로 변환한다.
- 이전 Phase에서 위임한 주제: 7-7에서 위임한 코드 리뷰의 문장·설계 관점은 10-2에서 본격적으로 다룬다. 8-5~8-6에서 위임한 프로젝트 기획·ADR·렌더링 전략 의사결정은 10-1에서 실제 프로젝트 문서로 확장한다. 8-3/8-4의 성능·보안 리포트는 10-3의 포트폴리오 증거와 10-4의 면접 답변 근거로 재사용한다.
- Phase 10 이후 별도 교육 Phase는 없다. 따라서 문서 집필 시 "나중에 심화"로 미루기보다, 필요한 범위는 앞선 Phase 문서로 역참조하고 최종 산출물에 어떻게 반영되는지까지 닫아야 한다.
...
### 과제 A — 프로젝트 정의와 ADR 작성 (1주차, 10-1 병행)
...
### 과제 C — 포트폴리오, 이력서, 면접 답변으로 변환 (3주차, 10-3~10-4 병행)
...
## 5. 공통 집필 기준 (Phase 10 특화)

CLAUDE.md의 전 지침에 더해, Phase 10에서 특히 지킬 것:
...
- [x] 10-1 `01-project-guide.md`
- [x] 10-2 `02-code-quality-and-review.md`
- [x] 10-3 `03-portfolio-and-resume.md`
- [x] 10-4 `04-interview-prep.md`
- [x] `exercises/phase-10/` 과제 안내 문서
