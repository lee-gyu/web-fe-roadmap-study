...
- [x] `pnpm docs:build`가 성공한다.
- [x] 문서 페이지 하단에 한국어 “마지막 업데이트”가 표시된다.
- [x] 문서 페이지의 편집 링크가 GitHub의 해당 Markdown 파일 편집 화면으로 이동한다.
- [x] 상단 내비게이션에 GitHub 저장소로 이동하는 링크가 표시된다.
- [x] 홈 화면에서 전체 로드맵의 목적과 시작점을 한눈에 확인할 수 있다.
