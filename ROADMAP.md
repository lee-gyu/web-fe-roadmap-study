...
| 기간 | 본 과정 약 39주 이상 + 부록 선택 학습 (주 20시간 이상 학습 기준, 경력자의 배경지식에 따라 단축 가능) |
| 주력 스택 | HTML / CSS / JavaScript / TypeScript / React / Next.js / Git |
| 산출물 | Phase별 실습 과제 + 성능·구조 분석 리포트 + Git 운영 플레이북 + 운영 대시보드·장애 분석 리포트 + 최종 포트폴리오 프로젝트 2개 이상. 부록 A는 상황별 사고법 레퍼런스, 부록 B는 아키텍처 비교표·선택 ADR·진화 경로 리포트, 부록 C는 반응성 실행 추적·CSR/SSR 비교 리포트와 SolidStart 통합 앱·설계 노트로 활용 |
...
flowchart LR
    ...
    P10 --> P10A["Phase 10a<br/>프론트엔드 관측 가능성과<br/>프로덕션 장애 진단"]
    P10A --> P11["Phase 11<br/>AI 에이전트 활용<br/>워크플로와 하네스"]
...
| 10a | 프론트엔드 관측 가능성과 프로덕션 장애 진단 | 2주 | 운영 대시보드, 장애 분석·대응 리포트 |
...
### Phase 10a — 프론트엔드 관측 가능성과 프로덕션 장애 진단 (2주)

**선정 이유**: Phase 8은 개발·실험 환경의 성능 계측을, Phase 10은 프로젝트 배포를 다루지만, 배포 후 실제 사용자에게 발생한 오류와 성능 회귀를 발견하고 릴리스·브라우저·네트워크·API 계층 중 원인을 좁히는 운영 과정은 비어 있다. 프론트엔드 엔지니어가 구현을 넘어 서비스 품질을 책임지려면 이 피드백 루프가 필요하다.

**학습 목표**: 로그·메트릭·트레이스와 실제 사용자 모니터링(RUM)의 역할을 구분하고, 사용자 영향 중심의 지표와 알림을 설계한다. 오류·성능 신호를 릴리스와 연결해 원인을 진단하고, 완화·롤백·재발 방지까지 근거를 남길 수 있다.

**도움이 되는 점**: 재현하기 어려운 사용자 오류를 증거로 추적하고, 평균값이나 알림 개수 대신 영향도에 따라 개선 우선순위를 정할 수 있다. 프론트엔드·백엔드·플랫폼 사이의 책임 공방을 줄이고 장애 대응과 회고를 공통 언어로 수행하는 데 도움이 된다.

| # | 문서 | 주요 내용 |
|---|------|----------|
| 10a-1 | `docs/phase-10a/01-observability-mental-model.md` | 관측 가능성과 모니터링의 차이, 로그·메트릭·트레이스의 역할, 브라우저와 서버 요청을 연결하는 상관관계 ID, 수집 범위·샘플링·개인정보·비용의 트레이드오프 |
| 10a-2 | `docs/phase-10a/02-errors-rum-and-release-health.md` | 전역 오류와 비동기 실패 수집, source map과 릴리스 식별, 오류 그룹화, RUM과 Core Web Vitals 분포, 사용자 영향 기반 SLI·SLO·알림 설계 |
| 10a-3 | `docs/phase-10a/03-production-diagnostics-and-incidents.md` | 오류·성능 회귀를 릴리스·브라우저·네트워크·API 계층으로 좁히는 진단 절차, 탐지→분류→완화→롤백→복구 흐름, 타임라인과 재발 방지 중심의 장애 회고 |

**실습 과제**: Phase 10 프로젝트에 최소한의 오류·성능 텔레메트리를 연결하고 운영 대시보드와 알림 기준을 만든다. 의도적으로 오류 또는 성능 회귀를 주입해 탐지·원인 추적·완화 또는 롤백을 수행한 뒤, 사용자 영향·증거·재발 방지 조치를 담은 장애 분석 리포트를 작성한다.
...
**학습 목표**: hacker-laws의 법칙과 원칙을 암기 목록이 아니라 상황을 해석하는 렌즈로 재분류한다. 각 법칙은 절대 명제가 아니라 특정 맥락에서 유용한 휴리스틱(heuristic)이며, 서로 충돌할 수 있고 조직·제품·시스템의 규모에 따라 가중치가 달라진다는 전제를 둔다.
...
│   ├── phase-10a/          # 프론트엔드 관측 가능성과 프로덕션 장애 진단
...
문서 작성 진행 상황과 미완료 TODO는 [PROGRESS.md](PROGRESS.md)에서 추적합니다.
